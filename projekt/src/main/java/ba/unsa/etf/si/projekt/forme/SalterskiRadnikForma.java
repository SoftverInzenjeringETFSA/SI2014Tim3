...
public class SalterskiRadnikForma implements ActionListener{
	...
	private JRadioButton jednosmjernaProdaja;
	...
	private void initialize() {
		...
		jednosmjernaProdaja = new JRadioButton("Jednosmjerna");
		...
		JLabel label_21 = new JLabel("Ime:");
		label_21.setHorizontalAlignment(SwingConstants.TRAILING);
		label_21.setBounds(308, 33, 65, 14);
		...
		rezervacijeList = new JList();   //popuni listu sa postojecim rezervacijama
		...
		rezervisiBtn = new JButton("Rezerviši");
			...
			public void actionPerformed(ActionEvent arg0) { //dodavanje rezervacijeee
				...
				try
				{
					...
					if(d1==0)
					{
						...
						if (vrijeme.length != 2) {
							JOptionPane.showMessageDialog(rezervisiBtn, "Vrijeme mora biti u formatu sati:minute!");
							return;
						} else if (!Validacija.jeInt(vrijeme[0]) || !Validacija.jeInt(vrijeme[1])) {
